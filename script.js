...
document.body.appendChild( renderer.domElement );
 ...
 var light = new THREE.DirectionalLight( new THREE.Color("red"), 1);
...
class JoyStick{
  constructor(options){
    ...
    thumb.style.cssText = "display: none;";
...
function updateDrive(forward = js.forward, turn = js.turn) {
  ...
  if (forward != 0) {
    mesh.translateZ(-force); // Move the player forward along its local z-axis in the world
  }
  mesh.rotateY(steer); // Rotate the player around its y-axis for steering
}
